...
/**
 * An utility metric based on the Soundex from the SimMetrics library.
 * @author Bruno P. Kinoshita
 * @since 0.1
 * @see uk.ac.shef.wit.simmetrics.similaritymetrics.Soundex
 */
public class FoneticaPortuguesaMetric {

	private final AbstractStringMetric internalStringMetric;
	
	/* default */ FoneticaPortuguesaMetric() {
		internalStringMetric = new JaroWinkler();
	}
	
	public Double getSimilarity(String from, String to) {
		String foneticaFrom = getFonetica(from);
		String foneticaTo = getFonetica(to);
		return (double) internalStringMetric.getSimilarity(foneticaFrom, foneticaTo);
	}

	private String getFonetica(String str) {
		final String replaced = str.toUpperCase();
		return new EncoderLetterByLetter(replaced).encode(); // TBD: reutilize the same object?
	}
	
	public static void main(String[] args) {
		FoneticaPortuguesaMetric metric = new FoneticaPortuguesaMetric();
		double d = metric.getSimilarity("Cibele", "Sibele");
